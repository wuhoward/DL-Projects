...
* **svgs/**: Equations used in this report
...
<a id='Image'></a>
#### Image
The raw image sizes vary. Simply resizing an image to square may alter the original aspect ratio. Fortunately, the flower itself usually locates in the center of the image, so we crop the longer side of the image. Then do the reshaping. Take the original paper as reference, we modify our preprocessing flow as follows:

1.	Center crop to square with side lengths equal to the shorter sides by **tf.image.resize_image_with_crop_or_pad**
2.	Scale each sides to 76/64 times as large as the target image’s side lengths
3.	Random flip horizontally by **tf.image.random_flip_left_right**
4.	Random crop to the target image size by **tf.random_crop**
5.	Normalize each pixel to \[-1, 1\]
...
The most significant difference between StackGAN and GAN-INT-CLS is the Conditioning Augmenetation process before feeding text embeddings into the generator. This innovation comes from the idea of variational autoencoder (VAE), where the latent space is mixed with random pertubations. These pertubations can be generated by a learned mean vector <img alt="$\mu$" src="svgs/07617f9d8fe48b4a7b3f523d6730eef0.png?invert_in_darkmode" align="middle" width="9.867990000000004pt" height="14.102549999999994pt"/> and a learned variance vector <img alt="$\Sigma$" src="svgs/813cd865c037c89fcdc609b25c465a05.png?invert_in_darkmode" align="middle" width="11.827860000000003pt" height="22.381919999999983pt"/>. By adapting to this noise in a text embedding, the model becomes more robust.
For reversed image caption, let the text embedding be <img alt="$\varphi_t$" src="svgs/9b0e1ecc9806900220f713c919089e97.png?invert_in_darkmode" align="middle" width="15.660480000000003pt" height="14.102549999999994pt"/>. We randomly sample latent variables from the distribution <img alt="$\mathcal{N}(\mu(\varphi_t),\Sigma(\varphi_t))$" src="svgs/8548e0ee0807f1f43d5a6473bebe5bf6.png?invert_in_darkmode" align="middle" width="116.05902pt" height="24.56552999999997pt"/> as the representations of the original sentence. Equivalently, we get a bunch of training data points for each sentence. To avoid small learned variances, and make the distribution closer to a normal distribution, the KL divergence between the distributions should be minimized. According to [Auto-Encoding Variational Bayes](https://arxiv.org/abs/1312.6114), an extra term should be added to the generator loss:
...
**Simple Version:**  
It performs downsampling through 4 strided-2 convolutional layers, where the output feature map is 0.5× higher, 0.5× wider and 2× deeper than the previous layer. The replicated text embeddings are stacked along the channel dimension before being concatenated with the final output of the downsampling layers. Lastly, it computes the logits by a window-size convolutional layer (equvalent to a fully connected layer).
...
### Loss Function
Different from the vanilla generator loss, an additional KL divergence loss is needed as afrontmentioned. As for the discriminator loss, apart from a real loss term (real image with real text) and a fake loss term (fake image with real text), a wrong loss term (real image with fake text) is also needed; otherwise, the generator may generate an image that seems real but has nothing to do with the provided captions. In practice, we shuffle the mapping relationships between images and captions instead of generating fake captions.  
[Back to Top](#Top)
...
#### Training Details
1.	Following the practice of GAN-INT, each sample of sentence embeddings is the average vector of 4 random captions out of the 10 captions. Therefore, the total samples per image is <img alt="$C_4^{10}$" src="svgs/607f6918b970d4240905190f3b810bcc.png?invert_in_darkmode" align="middle" width="25.936844999999998pt" height="26.70657pt"/>.
2.	Using of **tf.layers.batch_normalization** automatically adds two varibles, moving mean and moving variance to **tf.global_variables**. They need to be explicitly added to the variable list of the Saver to be saved and reuse in the future. Also, the update operations for them are in **tf.GraphKeys.UPDATE_OPS**, so **tf.GraphKeys.UPDATE_OPS** should be included into the control dependencies.
...
4.	We discovered that using only uni-skip embedding of Skip-Thought generates images with greyish colors. So, we decided to use the 2400-dimensional bi-skip vector with the 200-dimensional vector generated from the embedding matrix in seq2seq.

<a id='Evaluation'></a>
#### Evaluation
Inception score is one of the metric to evaluate the quailty of GAN-generated images. It takes advantage of an inception network pretrained on ImageNet. A "good model" possesses two attributes: 
...
#### Complex Model
The original StackGAN paper use the complex generator and discriminator. Nevertheless, in our experiments, we found out that the generated images were more unstable, the colors were more unnatural and the shapes were smoother. Furthermore, mode collapse happened easily after long enough training. The result is shown below:
...
After 595 epochs, the good images become better, while the broken images stay the way they were. We believe caption 7 and caption 8 captions are too complicated for our model to generate desirable images. For example, Caption 7 uses an adjective clause for description. Caption 8 uses a longer sentence with details and rare words:
...
After 400 epochs of Stage II training, the flowers become more realistic. For caption 2, the "dark lines" parts of flowers, which are ignored in Stage I, show up. For caption 7, the images are more like flowers.
